...
def addRankingToExperts(expertNames, expertsRankings, playerWithRankings):
	...
	for expert in expertNames:
		...
		i += 1
def addRankingToExpertsRB(expertNames, RBexpertsRankings, playerWithRankings):
	playerName = playerWithRankings[1].strip()
	i = 3
	first = playerWithRankings[i].strip()
	# Skip the injury field if there is one
	injuryTypes = ['P', 'Q', 'D', 'O', 'IR', 'SSPD']
	if first in injuryTypes:
		i = 5
	else:
		i = 4
	for expert in expertNames:
		if playerWithRankings[i].strip() == 'SSPD':
			RBexpertsRankings[expert].append((playerName, float('inf')))
		if playerWithRankings[i].strip() == 'NR':
			RBexpertsRankings[expert].append((playerName, float('inf')))
		elif expert == "Avg":
			RBexpertsRankings[expert].append((playerName, float(playerWithRankings[i])))
		else:
			RBexpertsRankings[expert].append((playerName, int(playerWithRankings[i])))
		i += 1
...
def getExpertRankingsQB(week, browser):
...
def getExpertRankingsRB(week, browser):
	
	url = "http://espn.go.com/fantasy/football/story/_/page/15ranksWeek%sRB/fantasy-football-week-%s-running-back-rankings" %(week, week)
	...
	RBexpertsRankings = {}
	for expert in expertNames:
		RBexpertsRankings[expert] = []

	for i in headers:
		playerWithRankings = i.get_text(" | ").split("|")
		# (player name, ranking)
		addRankingToExpertsRB(expertNames, RBexpertsRankings, playerWithRankings)

	for expert in RBexpertsRankings.keys():
		RBexpertsRankings[expert].sort(key= lambda tup: tup[1])

	# printRankings(expertsRankings)
	# browser.quit()
	return RBexpertsRankings

def getExpertRankingsWR(week, browser):
	
	url = "http://espn.go.com/fantasy/football/story/_/page/15ranksWeek%sWR/fantasy-football-week-%s-wide-receiver-rankings" %(week, week)
...
def getActualRankingsQB(week, browser):
...
def getActualRankingsRB(week, browser):
	url = "http://games.espn.go.com/ffl/leaders?&scoringPeriodId=%s&seasonId=2015&slotCategoryId=2" % week
	browser.visit(url)
...
def getActualRankingsWR(week, browser):
	url = "http://games.espn.go.com/ffl/leaders?&scoringPeriodId=%s&seasonId=2015&slotCategoryId=4" % week
	browser.visit(url)
...
def expertScore(expertRankings, actualRankings):
	...
	score = 0.0
	processed = []
	print "EXPERT RANKS:  " + str(expertRankings)
	print "ACTUAL RANSK:  " + str(actualRankings)
...
def expertScoreRB(expertRankings, actualRankings):
	# expertRankings: (player name, ranking)
	# actualRankings: (player name -> (position rank, fantasy points))
...
def getScoresForExpertsRB(expertsRankings, actualRankings):
	scores = []
	for expert in expertsRankings.keys():
		score = expertScoreRB(expertsRankings[expert], actualRankings)
		scores.append((expert, score))
	return scores

def updateEspnExpertScores(week, browser, scores):
	expertsRankingsQB = getExpertRankingsQB(week, browser)
	actualRankingsQB = getActualRankingsQB(week, browser)
	weekScoresQB = getScoresForExperts(expertsRankingsQB, actualRankingsQB)
	for tup in weekScoresQB:
		scores[tup[0]].append(tup[1])

def updateEspnExpertScoresRB(week, browser, scores):
	expertsRankingsRB = getExpertRankingsRB(week, browser)
	actualRankingsRB = getActualRankingsRB(week, browser)
	weekScoresRB = getScoresForExpertsRB(expertsRankingsRB, actualRankingsRB)
	for tup in weekScoresRB:
		scores[tup[0]].append(tup[1])

def updateEspnExpertScoresWR(week, browser, scores):
	expertsRankingsWR = getExpertRankingsWR(week, browser)
	actualRankingsWR = getActualRankingsWR(week, browser)
	weekScoresWR = getScoresForExperts(expertsRankingsWR, actualRankingsWR)
	for tup in weekScoresWR:
		scores[tup[0]].append(tup[1])
...
# Quarterback Calculations
	...
	for expert in ["Berry", "Karabell", "Yates", "Cockroft", "Clay", "Avg"]:
		scores[expert] = []
	for i in range(5,6):
		print("Calculating ESPN expert QB scores for Week %s" %i)
	...
	print("Final Average QB Scores for Each Expert:")
	for final in finalScores:
		print(final[0] + " : " + str(final[1]))
# Running Back Calculations
	# RBscores = {}
	# for expert in ["Berry", "Karabell", "Yates", "Cockroft", "Clay", "Avg"]:
	# 	RBscores[expert] = []
	# for i in range(5,6):
	# 	print("Calculating ESPN expert RB scores for Week %s" %i)
	# 	updateEspnExpertScoresRB(i, browser, RBscores)
	# RBfinalScores = []
	# for expert in RBscores.keys():
	# 	RBexpertScoreMean = stats.mean(RBscores[expert])
	# 	RBfinalScores.append((expert, RBexpertScoreMean))
	# print("Final Average RB Scores for Each Expert:")
	# for final in RBfinalScores:
	# 	print(final[0] + " : " + str(final[1]))
# WR CALCulation
	# WRscores = {}
	# for expert in ["Berry", "Karabell", "Yates", "Cockroft", "Clay", "Avg"]:
	# 	WRscores[expert] = []
	# for i in range(5,6):
	# 	print("Calculating ESPN expert WR scores for Week %s" %i)
	# 	updateEspnExpertScoresWR(i, browser, WRscores)
	# WRfinalScores = []
	# for expert in WRscores.keys():
	# 	WRexpertScoreMean = stats.mean(WRscores[expert])
	# 	WRfinalScores.append((expert, WRexpertScoreMean))
	# print("Final Average WR Scores for Each Expert:")
	# for final in WRfinalScores:
	# 	print(final[0] + " : " + str(final[1]))
	browser.quit()
